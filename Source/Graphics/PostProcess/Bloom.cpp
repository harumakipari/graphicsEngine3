...
void Bloom::make(ID3D11DeviceContext* immediate_context, ID3D11ShaderResourceView* color_map)
{
	...
	bit_block_transfer->Blit(immediate_context, glow_extraction->shaderResourceViews[0].GetAddressOf(), 0, 1, gaussian_blur_downsampling_ps.Get());
	...
	bit_block_transfer->Blit(immediate_context, gaussian_blur[0][0]->shaderResourceViews[0].GetAddressOf(), 0, 1, gaussian_blur_horizontal_ps.Get());
	gaussian_blur[0][1]->Deactivate(immediate_context);
	immediate_context->PSSetShaderResources(0, 1, &null_shader_resource_view);

	gaussian_blur[0][0]->Clear(immediate_context, 0, 0, 0, 1);
	gaussian_blur[0][0]->Activate(immediate_context);
	bit_block_transfer->Blit(immediate_context, gaussian_blur[0][1]->shaderResourceViews[0].GetAddressOf(), 0, 1, gaussian_blur_vertical_ps.Get());
	gaussian_blur[0][0]->Deactivate(immediate_context);
	immediate_context->PSSetShaderResources(0, 1, &null_shader_resource_view);
	...
	for (size_t downsampled_index = 1; downsampled_index < downsampled_count; ++downsampled_index)
	{
		...
		bit_block_transfer->Blit(immediate_context, gaussian_blur[downsampled_index - 1][0]->shaderResourceViews[0].GetAddressOf(), 0, 1, gaussian_blur_downsampling_ps.Get());
		...
		bit_block_transfer->Blit(immediate_context, gaussian_blur[downsampled_index][0]->shaderResourceViews[0].GetAddressOf(), 0, 1, gaussian_blur_horizontal_ps.Get());
		gaussian_blur[downsampled_index][1]->Deactivate(immediate_context);
		immediate_context->PSSetShaderResources(0, 1, &null_shader_resource_view);

		gaussian_blur[downsampled_index][0]->Clear(immediate_context, 0, 0, 0, 1);
		gaussian_blur[downsampled_index][0]->Activate(immediate_context);
		bit_block_transfer->Blit(immediate_context, gaussian_blur[downsampled_index][1]->shaderResourceViews[0].GetAddressOf(), 0, 1, gaussian_blur_vertical_ps.Get());
		gaussian_blur[downsampled_index][0]->Deactivate(immediate_context);
		immediate_context->PSSetShaderResources(0, 1, &null_shader_resource_view);
	}
	...
	for (size_t downsampled_index = 0; downsampled_index < downsampled_count; ++downsampled_index)
	{
		shader_resource_views.push_back(gaussian_blur[downsampled_index][0]->shaderResourceViews[0].Get());
	}
